...
if (isset($_SESSION['loggedin']) && isset($_SESSION['rid']) && $_SESSION['rid']==1){	
	...
	} else {$tgl=date('YYYY-mm-dd');$tglakhir=date('YYYY-mm-dd');$keywords=''; }
?>
	<table border=0 cellpadding=0 cellspacing=0 style="border:solid 0px #000;color:#000">
	<tr>
		<td colspan='8' height='10'width='120'><div align='center'><h3>Laporan Harian Kasir</h3></div></td>
		<td><a href="?n=lapkasir_c" style="border: 1px solid #666; background-color:#fff">&nbsp;&nbsp;&nbsp;Cetak&nbsp;&nbsp;&nbsp;</button></a></td>
		<td width='10'></td>
	...
	<form name='lapkasir' Aksi='?n=lapkasir' method='POST'>
	<tr bgcolor="#F8F8F8">
		<td width='10'></td>
...
<?php
		...
		if (is_array(isset($uidN)?$uidN:null)) {
			foreach($uidN as $key => $value) {
				$qryp =mysql_query("SELECT users.uid,users.uname, modal.momodal,
						COUNT(DISTINCT penjualan.pjid) as 'nota',
