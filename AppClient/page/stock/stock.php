...
?>
	<form name='stok' action='?n=stock' method='POST'>
		...
		<div id="MidPan">
			<div class="m1Pan">		
				...
				<tr height="25px" bgcolor="#000">
					...
					<td width='80'><div align='center'><h3>Satuan</h3></div></td>
					<td width='90'><div align='center'><h3>@ Harga Jual</h3></div></td>
					<td width='15'></td>
				</tr>
				</table>
			</div>
...
<?php
    $keywords	=isset($_POST['tsnama'])?$_POST['tsnama']:'';
				$q_barang=mysql_query("select a.*,b.pjsnama from barang a,pjsatuan b
							where a.bnama LIKE '%$keywords%' AND a.pjsid=b.pjsid order by a.bid");		  
...
?>
				...
				onmouseout="this.style.backgroundColor='#F8F8F8';this.style.color='#000'">
					...
					<td width='80'> <div align='right'>	<?php print $q_ba['pjsnama']?></div></td>
					<td width='90'> <div align='center'><?php print $q_ba['bjual']?></div></td>
				</tr>
<?php
				}
?>
			...
			<div class="m3Pan">
				...
				<tr>
					...
					<td width='3'>:</td>
					<td width='200'><input type="text" 	 name="tskode" id="idtskode" disabled='disabled' size=25 maxlength=25 tabindex=2 value="<?php print isset($bcode)?$bcode:0; ?>"><input name='bid' type='hidden' value="<?php print $bid ?>"></td>
